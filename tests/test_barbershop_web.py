...
def test_slots_then_a_booking_then_the_slot_is_gone(shop):
    ...
    booking = shop.ok("/api/book", {"day": day, "start": slots[0]["start"],
                                    "service": "haircut", "name": "Sipho",
                                    "phone": "082 123 4567"})["booking"]
...
def test_the_same_slot_twice_is_refused_politely(shop):
    ...
    body = {"day": day, "start": 10 * 60, "service": "haircut",
            "name": "Sipho", "phone": "0821234567"}
    shop.ok("/api/book", body)
    status, data = shop.request("/api/book", dict(body, name="Andile"))
    assert status == 400 and "taken" in data["error"]


def test_a_customer_finds_and_cancels_with_the_reference(shop):
    day = shop.tuesday()
    booking = shop.ok("/api/book", {"day": day, "start": 10 * 60,
                                    "service": "shave", "name": "Sipho",
                                    "phone": "0821234567"})["booking"]
    found = shop.ok("/api/lookup", {"ref": booking["ref"],
                                    "phone": "082 123 4567"})["booking"]
    assert found["service_name"] == "Shave"
    status, _ = shop.request("/api/lookup", {"ref": booking["ref"],
                                             "phone": "0829999999"})
    assert status == 404
    assert shop.ok("/api/cancel", {"ref": booking["ref"],
                                   "phone": "0821234567"})["booking"]["status"] \
        == "cancelled"
